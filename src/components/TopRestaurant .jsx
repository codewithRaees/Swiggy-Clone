  ...
  return (
    <div className=' max-w-screen-lg mx-auto py-5 flex flex-col px-3 md:px-0'>
      <div className="sliderTop flex justify-between px-1 py-2 items-center  ">
        <h1 className=' text-xl md:text-2xl font-extrabold font-[Poppins]'>Top restaurant chains in Mumbai</h1>
      ...
      <div className="card-main flex overflow-hidden justify-start ">
        ...
        { restaurantChains.map((restaurant,index)=> 
                   (
                    <div className=" flex flex-col gap-1 md:gap-5 px-1" style={{
                      transform: `translatex(-${slide * 100}%)`
                     }} key={index}>
  <div className="Test hover:scale-95 duration-300 transition-all md:w-72 w-48"><div className=''>
            <div className='w-48 md:w-72 md:h-48 h-32 relative   rounded-xl overflow-hidden'>
              ...
              <div className="inset-0 bg-gradient-to-t from-black/60 to-transparent heading absolute ">
                <span className='flex absolute bottom-0 px-2 pb-1 justify-items-end   md:text-2xl text-[16px] font-extrabold font-[Poppins] text-white'>
                  {restaurant.offer}
                </span>
              </div>
          ...
          <div className="restaurant-text ml-4 ">
            <div className="title flex flex-col">
              <h1 className='font-bold text-[14px] md:text-xl font-[Poppins] text-[#161A1F]'>{restaurant.title}</h1>
              <div className="flex rating font-[Poppins] font-bold  text-[#161A1F] text-[14px] md:text-[16px]"><span className='border bg-green-700 rounded-full w-5 justify-center items-center flex h-5 text-white '><MdStarRate className=''/></span> {restaurant.rating} . {restaurant.minTime}-30 {restaurant.maxTime}</div>
              <span className="address flex flex-wrap md:w-72 text-[#686b78] text-[14px] md:text-[16px] font-[Poppins] leading-5">{restaurant.name}
